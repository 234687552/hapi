...
export class Store {
    ...
    constructor(dbPath: string) {
        ...
        if (dbPath !== ':memory:' && !dbPath.startsWith('file::memory:')) {
            ...
            if (!existsSync(dbPath)) {
                try { const fd = openSync(dbPath, 'a', 0o600); closeSync(fd) } catch {}
            }
        }
    ...
    private migrateLegacySchemaIfNeeded(): void {
        ...
        if (hasDaemon && !hasRunner) this.migrateFromV1ToV2()
    }

    private migrateFromV1ToV2(): void {
        const columns = this.getMachineColumnNames()
        if (columns.size === 0) throw new Error('SQLite schema missing machines table for v1 to v2 migration.')
        const hasDaemon = columns.has('daemon_state') && columns.has('daemon_state_version')
        const hasRunner = columns.has('runner_state') && columns.has('runner_state_version')
        if (hasRunner && !hasDaemon) return
        if (!hasDaemon) throw new Error('SQLite schema missing daemon_state columns for v1 to v2 migration.')
        try {
            this.db.exec('BEGIN')
        ...
        try {
            this.db.exec('BEGIN')
            this.db.exec(`CREATE TABLE machines_new (
            ...
            this.db.exec(`INSERT INTO machines_new SELECT id, namespace, created_at, updated_at,
                metadata, metadata_version, daemon_state, daemon_state_version, active, active_at, seq FROM machines`)
            ...
            throw new Error(`SQLite schema migration v1->v2 failed: ${error instanceof Error ? error.message : String(error)}`)
        }
    }

    private migrateFromV2ToV3(): void { return }
    ...
    private assertRequiredTablesPresent(): void {
        ...
        if (missing.length > 0) {
            throw new Error(`SQLite schema is missing required tables (${missing.join(', ')}).`)
        }
    }
    ...
    private buildSchemaMismatchError(currentVersion: number): Error {
        const location = (this.dbPath === ':memory:' || this.dbPath.startsWith('file::memory:'))
            ? 'in-memory database' : this.dbPath
        return new Error(
            `SQLite schema version mismatch for ${location}. Expected ${SCHEMA_VERSION}, found ${currentVersion}.`
        )
    }
}
